      ...
      <template v-if="detail">
        <div class="head">
          ...
          <div class="info">
            ...
            <div v-if="detail.alt_title && detail.alt_title !== detail.title" class="alt">{{ detail.alt_title }}</div>

            <div class="meta-grid">
              <div v-if="detail.score != null" class="score">★ {{ Number(detail.score).toFixed(1) }}</div>
...
const summaryText = computed(() => (detail.value?.summary || '').trim())
...
.score { color: var(--brand); font-weight: 700; font-size: 16px; }
