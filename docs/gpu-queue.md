...
| 项 | 内容 |
|---|---|
| 任务 | dev 档 30 epoch 四组复核（M2.6b 口径确认） |
| 启动时间 | 2026-09-14 16:16 |
| 进程 | 串行 4 组，脚本见下方 §2.1 |
| 日志 | `logs/dev_m26b_base.log`（第 1 组，正在写）/ `_add` / `_mi_base` / `_mi_add` |
| 进度（16:57） | 第 1/4 组 epoch 16/30，实测 **159 s/epoch** |

**时间线（按实测 30.1 ms/step 折算）**：

| 顺序 | 组 | 预计完成 |
|---|---|---|
| 1/4 | `sasrec` 纯行为 | ~17:36 |
| 2/4 | `sasrec` + 内容（add） | ~18:57 |
| 3/4 | `multi_interest` 纯行为 | ~20:38 |
| 4/4 | `multi_interest` + 内容（add） | ~22:20 |
