...
| CPU | Intel 16 逻辑核 | — |
| GPU | **NVIDIA RTX 2060 / 6 GB / 驱动 616.92** | 驱动已升级；⚠️ 当前 `nvlddmkm.sys` 写坏导致设备 Code 52，见 progress.md §3.3 |
| 内存 | 31.7 GB | 加载 `ratings.npy`（6.9 GB）前建议先释放内存 |
| 磁盘 | F: 余 66 GB | 充足 |
| Python | 3.11.16（conda env `py3_11`） | `E:\tools\anaconda\envs\py3_11\python.exe` |
| PyTorch | **2.14.0+cu130**（CUDA 13.0） | ⚠️ PyPI 默认源只给 `+cpu` 轮子，必须显式指定 CUDA 索引 |
