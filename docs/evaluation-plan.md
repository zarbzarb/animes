...
> 上表约 26 次训练按「每档跑满 30 epoch」的保守口径合计 **1.66 天**
> （实际有早停 patience=10，会更短）。各档单次耗时：`smoke` 3 分钟 / `debug` 27 分钟 /
> `dev` 1.13 h / `main` 2.27 h / `full` 22.7 h。
>
> 早期文档的「~77 h / 72 次训练」按 RTX 3090 估 —— **已废弃，不得再引用**。
> 完整实测表、复现命令与三条调参发现见 [progress.md](progress.md) §5.0。
...
| 档位 | 用户比例 | 用户数 | epoch 上限 | 种子数 | 实测 sec/epoch | 实测 30 epoch | 用途 |
|---|---|---|---|---|---|---|---|
| `smoke` | 0.5% | 6,533 | 3 | 1 | 13 s | 3 分钟 | 改完代码先跑，确认链路没断 |
| `debug` | 2% | 26,134 | 30 | 1 | 54 s | 27 分钟 | 判断改动方向（涨/跌），不看绝对值 |
| `dev` | 5% | 65,335 | 30 | 1 | 136 s | 1.13 h | HPO 细搜、消融、基线对比 |
| `main` | 10% | 130,669 | 30 | 3 | 273 s | 2.27 h | E1 主对比 + E2 消融的正式数字 |
| `full` | 100% | 1,306,691 | 30 | 1 | 2,721 s | 22.7 h | 附录的全量规模验证，时间不够可放弃 |
...
| 实验组 | 训练次数 | 档位 | 单次耗时 | 小计 |
|---|---|---|---|---|
| E1 整体对比 | 9 | `main`（10%，30 epoch） | 2.27 h | **0.85 天** |
| E2 消融 | 2 | `dev` | 1.13 h | 2.26 h |
| E3 冷启动 | 2 | `dev` | 1.13 h | 2.26 h |
| E4 分题材 | 0 | 复用 E2 | — | — |
| HPO 粗搜 | 8 | `smoke`（0.5%，3 epoch） | 0.01 h | 5.3 分钟 |
| HPO 细搜 | 3 | `dev` | 1.13 h | 3.40 h |
| 补充实验 | 10 | `dev` | 1.13 h | 11.32 h |
| **合计** | **约 26** | — | — | **≈ 1.66 天** |
...
- 上表是**纯计算**吞吐，不含 DataLoader 取数与滑窗组装开销；端到端以训练日志首个 epoch 为准。
