...
> ⚠️ **早期文档中的「~77 h / 72 次训练」是按 RTX 3090 估的，不可作为排期依据。**
> 本机为 RTX 2060 6GB 且纯本地、无云算力；阶段一实测每 epoch 的全量训练样本为
> **109,081,471**（约 1.09 亿）。不分档、不复用则总时长为「年」量级。
> 环境实况与决策记录见 [progress.md](progress.md) §3、§5、§6。
...
| 评估开销 | 评估降频 + 评估子集 | 评估本身就要 1.32 亿次打分/轮，不治理会超过训练时间 |

#### 6.6.2 五个档位

| 档位 | 用户比例 | 用户数 | epoch 上限 | 种子数 | 用途 |
|---|---|---|---|---|---|
| `smoke` | 0.5% | 6,533 | 3 | 1 | 改完代码先跑，几分钟确认链路没断 |
| `debug` | 2% | 26,134 | 30 | 1 | 判断改动方向（涨/跌），不看绝对值 |
| `dev` | 5% | 65,335 | 30 | 1 | HPO 细搜、消融、基线对比 |
| `main` | 10% | 130,669 | 30 | 3 | E1 主对比 + E2 消融的正式数字 |
| `full` | 100% | 1,306,691 | 30 | 1 | 附录的全量规模验证，时间不够可放弃 |

> 用户在 [AskUserQuestion] 中的决策：**纯本地不花钱 + 先抽样、有余力再全量 + 优先牺牲种子数**。
> 因此 `main` 档定为 10% 而非全量，且只有它跑 3 个种子 —— 种子数是对结论影响最小的取舍项。
...
#### 6.6.4 算力预算（待 M2.0 实测校准）

单次训练耗时 ≈ **T × epoch 数 × 用户比例**，其中 T 为全量单 epoch 的实测耗时。
M2.0 会用 `scripts/bench_throughput.py` 在 5 万用户子集上实测 T，再代入下表。

| 实验组 | 训练次数 | 档位 | 累计耗时 ≈ |
|---|---|---|---|
| E1 整体对比 | 9 | `main`（10%，30 epoch） | 27 T |
| E2 消融 | 2 | `dev`（5%，30 epoch） | 3 T |
| E3 冷启动 | 2 | `dev` | 3 T |
| HPO 粗搜 | 8 | `smoke`（0.5%，3 epoch） | 0.12 T |
| HPO 细搜 | 3 | `dev` | 4.5 T |
| 补充实验 | 10 | `dev` | 15 T |
| **合计** | **34** | — | **≈ 52.6 T** |

> 举例：若实测 T = 3 小时，则总训练时长约 **158 小时 ≈ 6.6 天**（不含评估与调试）。
> ⚠️ 以上全部为**待标定的估算**，不是实测结果；T 未测出前不得写入论文的耗时描述。
