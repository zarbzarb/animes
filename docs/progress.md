...
| `gru4rec.py` 经典序列模型（H=64，1 层 GRU） | **是** | 1,033,088 | ✅ main/val+test 已出正式数字（val 0.9658/0.7966；test 0.9709/0.7878）；×3 种子续后 |
...
main 档 val+test 均已出（test：hr@10 0.9709 / ndcg@10 0.7878 / mrr 0.7299，n=129,382，剔除泄漏 1,287 条，训练 2,950 s）；**×3 种子（42/2024/2025）续后**。
...
| 单测：**825 例全绿**（M2 的 421 + M3 新增 404） | ✅ |
| **M3.6 HTTP 层测试**：`tests/test_api/` 在**真应用 + SQLite 内存库**上跑（不 mock 网关、不跑 lifespan 免 GPU）——44 端点信封全审计、鉴权矩阵全审计（公开/仅管理员白名单显式固化）、行为回归 172 例 | ✅ |
| **M3.7 前端正式工程**（`frontend/`，Vue 3 + Vite + Element Plus + ECharts，对齐 rq.md 技术选型）：用户端 5 页（登录注册/首页推荐含双路分数条+推荐理由/追番管理/新番专区/兴趣分析含 ECharts 雷达+漂移趋势+漂移点）+ 管理端 3 页（番库管理含下架/用户管理/效果监控含 Agent 健康与指标图）+ 追番助手（SSE 流式对话，解析 tool_call/chunk/cards 帧）；vite proxy `/api` → 8000 免 CORS；统一信封客户端（40101 自动登出、60401 降级警告条）；`npm run build` 通过，登录/推荐/雷达实测链路通 | ✅ |
...
**M3.6 全端点审计又抓出两个**（同样带回归 + 变异检验；冒烟没覆盖到，恰好证明审计的价值）

8. **`GET /records/timeline` 必 500**：接口一直在给网关的
   `user_genre_time_series()` 传不存在的 `limit` 参数，未捕获 TypeError。
   修复：网关加 `limit`（只截**周期数**、跨题材共享窗口，A6 默认不限零影响）。
   教训：统一异常处理器会把 500 包成**合法信封**（`code=50001`），
   所以"信封完整"类测试对 500 是盲的 —— 审计测试必须显式断 `status < 500`。
9. **"下架"只挡了四条读路**：`set_anime_offline()` 置 `is_online=0` 后，
   列表/计数/热门/批量都过滤它，但详情页、相似接口、加追番的存在性检查
   走的是单查 `get_anime`（不过滤）——管理员点"下架"对用户等于没下架。
   且 `deps.require_anime` 这个本该统一口径的依赖**从未被任何路由引用**
   （死代码制造了"有统一依赖"的假象）。已删死代码，新增
   `deps.is_visible_anime()` 收敛四处用户侧判定；admin 编辑路径不受限。

**M3.6 的其他产出**：`response.py` 两处 docstring 修正 —— fastapi 0.141.1 起
`include_router` 不再把子路由摊平进 `app.routes`（包成无 `path` 的
`_IncludedRouter`），"遍历 `app.routes` 找子路由"会静默收出 **0 条**
...
1. **验证码**：新增 `GET /auth/captcha`（`server/core/captcha.py`，纯 Python 生成
   扭曲 SVG，零依赖）+ 登录接口校验。一次一密（先删后比，防重放）、大小写不敏感、
   TTL 5 分钟、字符集去易混字形。服务端默认宽松（不带放行，冒烟/CI 依赖），
   生产 `AUTH_CAPTCHA_STRICT=true` 后必填 —— 两档都有测试锁定（5 例 + 变异检验）。
2. **"密码错误无提示"是真 bug**：登录失败走 HTTP 401 → axios error 拦截器对
   40101 只做"登出+跳登录"，人已在登录页 → 静默无反应。修复：登录页上的
   40101 只弹提示不跳转（跳转会成循环），使用途中失效才踢回登录。
3. **布局**：el-tabs 换成滑动分段控件 + 左右分栏卡片（左品牌/右表单）。
4. **注册**：加确认密码（两次一致性校验）+ 密码强度条（弱/中/强）。
   校验错误就地显示在表单项下，不再依赖弹窗。
