...
> **快照时间**：2026-09-12 20:30 ｜ **最新完成里程碑**：**M2.6 排序侧内容融合（实现+验证完毕，结论：后验融合无增益，候选侧拼接转 M2.6b）** ｜ **下一里程碑**：M2.6b 候选侧拼接
...
| 阶段 2　核心算法与对比实验 | 35% | 🔵 **进行中 75%** | **M2.0 ~ M2.5 完成 + M2.6 排序侧融合完成**：SASRec 基座 lr=0.001 定稿；多兴趣胶囊跑通（0.7624 vs 0.7871 同预算）；⚠️ 热度先验主导指标；⚠️ 后验内容融合实测无增益（纯内容 0.174 vs 行为 0.788，冷启动 5:5 也不比 7:3 好）——E2_3 如实报告，候选侧拼接转 M2.6b |
...
| **M2.6** 🔵 | 内容融合 | 排序侧 7:3 加权（**已实现并验证**）；候选侧拼接（待 M2.6b）；冷启动权重切 5:5 | `models/content_encoder/fusion.py`、`scripts/eval_content_fusion.py` | ✅ `fusion_score()` 契约达成；**排序侧后验融合实测无增益**（见 §4.1 M2.6），E2_3 将以此口径如实报告 |
| **M2.6b** | 候选侧拼接 | 内容向量投影后拼进物品表示、端到端训练（区别于后验打分融合） | `models/content_encoder/model.py` | debug 档与 M2.4 基线同预算对照，验证结构级融合是否优于后验融合 |
...
推荐执行顺序即上表自上而下。**M2.0–M2.5 及 M2.6 排序侧已完成，下一步 M2.6b（候选侧拼接）**，环境已无阻塞。
...
增量信息。**这否定的只是"后验融合"，不是内容通路本身**；候选侧拼接（内容向量
作为输入特征端到端训练，M2.6b）是下一验证路径，E2_3/E2_4 的正式结论以其为准。
